...
new_Pi = []
for element in Pi:
    new_Pi.append(int(element))

answer = 0
time_dict = {}
for i in range(N):
    time_dict.update({i+1:new_Pi[i]}) # {1:3, 2:1, 3:4, 4:3, 5:2}
sorted_time_dict = sorted(time_dict.items(), key=lambda x:x[1]) # {1:2, 2:5, 3:1, 3:4, 4:3}
for i in range(N):
    answer += (N-i)*(sorted_time_dict[i][1])
print(answer)